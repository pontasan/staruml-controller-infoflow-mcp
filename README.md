# StarUML Controller MCP Server — Information Flow Diagram

An MCP (Model Context Protocol) server specialized for **Information Flow Diagrams**. Enables AI assistants like Claude to programmatically create and manage information flow diagrams — information items and information flows in [StarUML](https://staruml.io/).

<p align="center">
  <img src="images/main.jpg" alt="EC Platform - Microservices Information Flow Diagram" width="800">
</p>

## Architecture

<p align="center">
  <img src="images/architecture.svg" alt="Architecture: Claude ↔ MCP (stdio) ↔ staruml-controller-infoflow-mcp ↔ HTTP REST API ↔ StarUML" width="800">
</p>
...
```bash
git clone https://github.com/pontasan/staruml-controller-infoflow-mcp.git
cd staruml-controller-infoflow-mcp
npm install && npm run build
```
...
4. The HTTP server starts — all Information Flow Diagram tools become available via MCP
...
```json
{
  "mcpServers": {
    "staruml-infoflow": {
      "command": "node",
      "args": ["/absolute/path/to/staruml-controller-infoflow-mcp/dist/index.js"]
    }
  }
}
```
...
```bash
claude mcp add staruml-infoflow node /absolute/path/to/staruml-controller-infoflow-mcp/dist/index.js
```
...
### Information Flow Diagram Tools

The core of this server — full CRUD for every Information Flow Diagram resource.

| Resource | Tools |
|---|---|
| **Diagrams** | `infoflow_list_diagrams`, `infoflow_create_diagram`, `infoflow_get_diagram`, `infoflow_update_diagram`, `infoflow_delete_diagram` |
| **Info Items** | `infoflow_list_info_items`, `infoflow_create_info_item`, `infoflow_get_info_item`, `infoflow_update_info_item`, `infoflow_delete_info_item` |
| **Information Flows** | `infoflow_list_information_flows`, `infoflow_create_information_flow`, `infoflow_get_information_flow`, `infoflow_update_information_flow`, `infoflow_delete_information_flow` |
